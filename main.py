import discord, pymongo, motor, json, time
...
class register(discord.ui.View):
    ...
    async def button_callback(self, button, interaction):
        db.users.insert_one(
            {"_id": interaction.user.id, "blacklist": [], "whitelist": [], "banned": False, "bantime": time.time()}
        )
...
class Users(discord.ui.View):
    ...
    async def select_callback(self, select, interaction):
        ...
        await interaction.response.send_message(f"Added {', '.join([user.display_name for user in select.values])} to your blacklist!", ephemeral=True)
